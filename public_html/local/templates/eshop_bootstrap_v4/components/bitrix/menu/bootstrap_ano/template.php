            ...
            foreach ($arResult["MENU_STRUCTURE"] as $itemID => $arColumns) {
                ...
                >
					...
					<span class="bx-nav-1-lvl-link-text">
                        ...
                        <? if (is_array($arColumns) && !empty($arColumns)):?> <i
                                class="bx-nav-angle-bottom"></i><?endif ?>
					</span>
                    </a>
                    <?
                    if (is_array($arColumns) && !empty($arColumns)) {
...
<script>
    BX.ready(function () {
        ...
        $('body').on('click', '.bx-menu-button-mobile', function () {
            $('.searchMobile').remove();
            $.ajax({
                url: '/bonus-shop/ajaxMobileMenu.php',         /* Куда отправить запрос */
                method: 'get',             /* Метод запроса (post или get) */
                dataType: 'json',          /* Тип данных в ответе (xml, json, script, html). */
                data: {action: 'mobileInsert'},     /* Данные передаваемые в массиве */
                success: function (data) {   /* функция которая будет выполнена после успешного запроса.  */
                    $('.bx-aside-nav').append(data.html);
                }
            });
